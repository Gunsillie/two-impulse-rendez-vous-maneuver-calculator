...
First, let us consider a simple case such as the one proposed in the assignment, and described by the table below.
...
Given these initial conditions, the program computes all positions of the chaser until the time of rendez-vous. The Figure \ref{plot:xz} shows the trajectory taken by the spacecraft only looking at its $x$ and $z$ coordinates ($y$ stays equal to zero during the whole trajectory because of the initial position of the chaser).
\begin{figure}[htp!]
  \centering
  \input{tikzplot}
...
\begin{figure}[htp!]
  ...
  \label{plot:xzt}
\end{figure}
The ellipse-shaped of the trajectory is coherent with the model used and the case studied where only $x_0$ is not equal to zero. We can also draw the evolution of both $x$ and $z$ coordinates as a function of time, as can be seen in Figure \ref{plot:xzt}.
